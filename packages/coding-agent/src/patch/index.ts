...
import { applyChunkEdits, type ChunkEditOperation, getChunkInfoForFile, resolveAnchorStyle } from "../tools/chunk-tree";
import {
...
function parseChunkTarget(target: string): ParsedChunkTarget {
	...
	return {
		selector: target.slice(0, hashIndex),
		crc: target.slice(hashIndex + 1).toUpperCase(),
	};
}
...
function isHashlineParams(
	...
	return "edits" in params && Array.isArray(params.edits) && (params.edits.length === 0 || "loc" in params.edits[0]);
}
...
function isChunkParams(params: ReplaceParams | PatchParams | HashlineParams | ChunkParams): params is ChunkParams {
	return "edits" in params && Array.isArray(params.edits) && params.edits.length > 0 && "target" in params.edits[0];
}
...
export class EditTool implements AgentTool<TInput> {
	...
	async execute(
		...
		if (this.mode === "chunk") {
			...
			enforcePlanModeWrite(this.session, path, { op: sourceExists ? "update" : "create" });
			...
			const assertChecksum = async (op: string, crc: string | undefined, selector: string): Promise<string> => {
				if (crc) return crc.toUpperCase();
				if (selector.length > 0 && sourceExists) {
					const resolved = await getChunkInfoForFile(resolvedPath, chunkLanguage, selector);
					if (resolved) {
						throw new Error(
